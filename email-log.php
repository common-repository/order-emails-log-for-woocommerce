...
class WOEL_Email_Log {
	...
	public function addFilterActions() {      
		...
		// saves order_id for plugin woocommerce-email-center
		add_filter( 'rp_wcec_send_email_immediately', array( &$this, 'for_email_plugin'), 10, 3 );
		add_filter( 'rp_wcec_schedule_email', array( &$this, 'for_email_plugin'), 10, 3 );  

        // saved order id for plugin automatewoo
        add_filter( 'automatewoo/workflow/mailer', array( &$this, 'for_automatewoo_plugin'), 30, 2 );
    ...
    // saved order id
    public function for_automatewoo_plugin( $mailer, $workflow ){
        if( isset($workflow->workflow->log->meta_cache['order_id']) ){
            $this->order_id = $workflow->workflow->log->meta_cache['order_id'];
        }
        return $mailer;
    }    
	
	// saved order id
	public function for_email_plugin( $boo, $t, $args ){
		if( isset($args['order'] )){
			$this->order_id = trim(str_replace('#', '', $args['order']->get_order_number()));  
		}
	    return true;
	}		
